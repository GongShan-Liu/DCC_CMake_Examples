...
@REM @REM 构建文件
"C:\Program Files\CMake\bin\cmake.exe" -DMAYA_SDK_LOCATION=%maya_sdk_path% -DMAYA_VERSION=%compile_maya_version% -DCMAKE_EXPORT_COMPILE_COMMANDS:BOOL=TRUE --no-warn-unused-cli -S %cd% -B %cd%/build/%compile_maya_version% -G "Visual Studio 15 2017" -T host=x64 -A x64 -DCMAKE_CONFIGURATION_TYPES=Release
